package com.example.jakub.arapp.bluetoothManager.connectionManager;
...
public class ConnectedBleDevice {
    ...
    private final String ADDRESS_PATTERN = "00001111-0000-1000-8000-00805f9b34fb";
    ...
    public void establishConnectionAndReceiveData() {
        connection = rxBleDevice.establishConnection(false)
                .flatMap(rxBleConnection -> rxBleConnection.setupNotification(UUID.fromString(ADDRESS_PATTERN)))
                .doOnNext(notificationObservable -> {
                })
                .flatMap(notificationObservable -> notificationObservable)
                .subscribe(
                        bytes -> sendBroadcast(ConnectedBleDeviceProvider.ACTION_NEW_DATA_AVAILABLE, rxBleDevice.getMacAddress(),
                        String.valueOf(ByteBuffer.wrap(bytes).order(ByteOrder.LITTLE_ENDIAN).getDouble())),
                        throwable ->
